...
error_arima <- train-fitted_values
# Menghitung MSE
...
if (nrow(data_ann) > 0) {
  ...
  if (length(pred_ann_denorm) != length(fitted_values)) {
    pred_ann_denorm <- head(pred_ann_denorm, length(fitted_values))
  }
}
...
final_pred <- fitted_values + pred_ann_fixed
...
```{r}
# 1. Fit model ARIMA
model_arima <- arima(test, order = c(10, 1, 1))
